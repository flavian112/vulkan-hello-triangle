...
bool pipeline_create(pipeline_t *pipeline, const device_t *device, const renderpass_t *renderpass) {
    ...
    VkShaderModule vertex_shader_module =
        pipeline_create_shader_module(device->vk_device, vertex_shader_spv_data, vertex_shader_spv_size);
    ...
    VkShaderModule fragment_shader_module =
        pipeline_create_shader_module(device->vk_device, fragment_shader_spv_data, fragment_shader_spv_size);
    ...
    VkResult res;
    res = vkCreatePipelineLayout(device->vk_device, &pipeline_layout_create_info, NULL, &pipeline->vk_pipeline_layout);
    if (res != VK_SUCCESS) {
    ...
    res = vkCreateGraphicsPipelines(
        device->vk_device, VK_NULL_HANDLE, 1, &graphics_pipeline_create_info, NULL, &pipeline->vk_pipeline);
    if (res != VK_SUCCESS) {
