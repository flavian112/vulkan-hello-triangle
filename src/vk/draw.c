...
draw_result_t draw_frame(const device_t *device,
    ...
    assert(*current_frame < sync->count);

    VkResult res;
    res = vkWaitForFences(device->vk_device, 1, &sync->in_flight[*current_frame], VK_TRUE, UINT64_MAX);
    if (res != VK_SUCCESS) {
        log_error("(DRAW) vkWaitForFences failed (%s).", vk_res_str(res));
        return DRAW_ERROR;
    }
    res = vkResetFences(device->vk_device, 1, &sync->in_flight[*current_frame]);
    if (res != VK_SUCCESS) {
        log_error("(DRAW) vkResetFences failed (%s).", vk_res_str(res));
        return DRAW_ERROR;
    }

    res = vkWaitForFences(device->vk_device, 1, &sync->present_done[*current_frame], VK_TRUE, UINT64_MAX);
    if (res != VK_SUCCESS) {
        log_error("(DRAW) vkWaitForFences failed (%s).", vk_res_str(res));
        return DRAW_ERROR;
    }
    res = vkResetFences(device->vk_device, 1, &sync->present_done[*current_frame]);
    if (res != VK_SUCCESS) {
        log_error("(DRAW) vkResetFences failed (%s).", vk_res_str(res));
        return DRAW_ERROR;
    }
    ...
    res = vkAcquireNextImageKHR(device->vk_device,
                                swapchain->handle,
                                UINT64_MAX,
    ...
    submit_info.pWaitSemaphores = &sync->image_available[*current_frame];
    submit_info.pWaitDstStageMask = &wait_stage;
    submit_info.commandBufferCount = 1;
    submit_info.pCommandBuffers = &commands->vk_buffers[*current_frame];
    submit_info.signalSemaphoreCount = 1;
    submit_info.pSignalSemaphores = &sync->render_finished[*current_frame];

    res = vkQueueSubmit(device->graphics_queue, 1, &submit_info, sync->in_flight[*current_frame]);
    if (res != VK_SUCCESS) {
    ...
    swapchain_present_fence_info.pFences = &sync->present_done[*current_frame];

    VkPresentInfoKHR present_info = {0};
    present_info.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
    present_info.pNext = &swapchain_present_fence_info;
    present_info.waitSemaphoreCount = 1;
    present_info.pWaitSemaphores = &sync->render_finished[*current_frame];
