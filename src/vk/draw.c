...
draw_result_t draw_frame(const device_t *device,
    ...
    VkResult res;
    res = vkWaitForFences(device->vk_device, 1, &sync->vk_fence_in_flight[*current_frame], VK_TRUE, UINT64_MAX);
    if (res != VK_SUCCESS) {
    ...
    res = vkWaitForFences(device->vk_device, 1, &sync->vk_fence_present_done[*current_frame], VK_TRUE, UINT64_MAX);
    if (res != VK_SUCCESS) {
        log_error("(DRAW) vkWaitForFences failed (%s).", vk_res_str(res));
        return DRAW_ERROR;
    }
    ...
    res = vkQueueSubmit(device->graphics_queue, 1, &submit_info, sync->vk_fence_in_flight[*current_frame]);
    if (res != VK_SUCCESS) {
